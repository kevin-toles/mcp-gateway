"""Script to write the AEI-7 test file."""

import pathlib

TARGET = pathlib.Path(__file__).resolve().parent.parent / "tests" / "unit" / "tools" / "test_amve_tools.py"
...
class TestAMVEServiceNames:
    ...
    def test_all_amve_tools_map_to_amve_service(self):
        ...
        for name in AMVE_TOOL_NAMES:
            ...
            assert _TOOL_SERVICE_NAMES[name] == "amve", (
                f"{name} maps to {_TOOL_SERVICE_NAMES[name]!r} not amve"
            )
